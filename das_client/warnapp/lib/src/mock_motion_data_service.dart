...
class MockMotionDataService implements MotionDataService {
  MockMotionDataService({String? motionData, this.samplingPeriod}) {
  ...
  void _parseMotionData(String? motionDataString) {
    ...
    for (final row in rows.skip(1)) {
      ...
      motionData.gyroscopeEvent = GyroscopeEvent(
        double.parse(data[2]),
        double.parse(data[3]),
        double.parse(data[4]),
        timestamp,
      );
      motionData.accelerometerEvent = AccelerometerEvent(
        double.parse(data[5]),
        double.parse(data[6]),
        double.parse(data[7]),
        timestamp,
