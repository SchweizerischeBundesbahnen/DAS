...
void main() {
  group('PersonalNotesViewModel', () {
    ...
    test('locationCode_whenServicePointChanges_thenLoadsPersonalNoteForCurrentLocation', () async {
      ...
      when(mockPersonalNotesRepository.findNote(servicePointA.locationCode)).thenAnswer((_) async => note);
      final personalNoteRegister = <PersonalNote?>[];
      final subscription = testee.personalNote.listen(personalNoteRegister.add);
      ...
      expect(testee.personalNoteValue, note);
      expect(personalNoteRegister, orderedEquals([null, note]));
    ...
    test('saveNote_whenSingleUse_thenPersistsNoteAndUpdatesValue', () async {
      ...
      expect(testee.personalNoteValue, savedNote);
    });
    ...
    test('deleteNote_whenNoNoteExists_thenDoesNothing', () async {
      // WHEN
      await testee.deleteNote();

      // THEN
      verifyNever(mockPersonalNotesRepository.deleteNote(any));
      verifyNever(mockPersonalNotesRepository.findAllNotes());
    });

    test('deleteNote_whenCurrentNoteExists_thenDeletesNoteAndClearsValue', () async {
      // GIVEN
      ...
      // THEN
      verifyNever(mockPersonalNotesRepository.findAllNotes());
      verify(mockPersonalNotesRepository.deleteNote(servicePointA.locationCode)).called(1);
      expect(testee.personalNoteValue, isNull);
    });
    ...
    test('deleteNote_whenDeletedNoteIsFootnote_thenReloadsAnnotations', () async {
      ...
      when(mockPersonalNotesRepository.findAllNotes()).thenAnswer((_) async => allNotes);
      when(mockPersonalNotesRepository.findNote(servicePointA.locationCode)).thenAnswer((_) async => allNotes.single);
      when(mockPersonalNotesRepository.deleteNote(servicePointA.locationCode)).thenAnswer((_) async {
        allNotes = [];
      });
      await emitJourney(_journey(data: [servicePointA]));
      await emitServicePoint(servicePointA);
