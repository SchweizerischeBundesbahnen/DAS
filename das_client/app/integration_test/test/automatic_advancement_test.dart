...
void main() {
  group('automatic advancement tests', () {
    patrolTest('check if automatic advancement is scrolling automatically', (tester) async {
      // Load app widget.
      await prepareAndStartApp(tester.tester);

      // load train journey by filling out train selection page
      await loadTrainJourney(tester.tester, trainNumber: 'T9');
    ...
    patrolTest('check scrolling after idle time', (tester) async {
      // Load app widget.
      await prepareAndStartApp(tester.tester);
      ...
      await tester.tester.dragUntilVisible(find.text('B1'), scrollableFinder, const Offset(0, 100));
    ...
    patrolTest('check scrolling to position if automatic scrolling gets enabled', (tester) async {
      // Load app widget.
      await prepareAndStartApp(tester.tester);
      ...
      await waitUntilNotExists(tester.tester, find.byKey(RouteChevron.chevronKey), maxWaitSeconds: 40);

      // Wait some more
      await tester.tester.pump(const Duration(seconds: 4));

      await startAutomaticAdvancement(tester.tester);
    ...
    patrolTest('check not scrolling if automatic advancement is off', (tester) async {
      // Load app widget.
      await prepareAndStartApp(tester.tester);
    ...
    patrolTest('check if automatic advancement is enabled by default', (tester) async {
      // Load app widget.
      await prepareAndStartApp(tester.tester);
    ...
    patrolTest('check sticky footer is displayed', (tester) async {
      // Load app widget.
      await prepareAndStartApp(tester.tester);
