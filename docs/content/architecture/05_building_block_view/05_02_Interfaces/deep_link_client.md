...
* `[]`: The list allows passing a whole tour [1..*] of the very same train driver. Entries must be provided in ascending chronological order (by departure time). The first entry represents the immediate/next train journey and is the one that will be opened.
