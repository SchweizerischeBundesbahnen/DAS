...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http, JwtAuthenticationConverter jwtAuthenticationConverter) {
        http.authorizeHttpRequests(authorize -> authorize
                ...
                .requestMatchers(API_SETTINGS, API_FORMATIONS, API_CUSTOMER_ORIENTED_DEPARTURE + "/**", API_NOTICES_MATCHES).hasAnyRole(UserRole.OBSERVER, UserRole.DRIVER)
                .requestMatchers(API_SETTINGS_APP_VERSION + "/**", API_LOCATIONS).hasRole(UserRole.ADMIN)
                .requestMatchers(API_NOTICE_TEMPLATES + "/**", API_SPECIAL_HOLIDAYS + "/**", API_NOTICES + "/**", API_COMPANIES)
