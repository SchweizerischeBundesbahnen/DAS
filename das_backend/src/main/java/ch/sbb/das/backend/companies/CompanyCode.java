 ...
 * <p>
 * Relates to teltsi_CompanyCode (according to SFERA). Format: exactly 4 upper-case alphanumeric characters, not {@code "0000"}, e.g. {@code "2185"}.
 * <p>
...
public record CompanyCode(@JsonValue @NonNull String value) {
    ...
    private static final Pattern COMPANY_CODE_PATTERN = Pattern.compile("(?!0000)[0-9A-Z]{4}");

    public CompanyCode {
        if (!COMPANY_CODE_PATTERN.matcher(value).matches()) {
            throw new IllegalArgumentException("CompanyCode must be 4 upper-case alphanumeric characters, not 0000");
        }
    }
