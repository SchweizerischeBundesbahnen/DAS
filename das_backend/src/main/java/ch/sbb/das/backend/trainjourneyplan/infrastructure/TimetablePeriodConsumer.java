...
import ch.sbb.das.backend.trainjourneyplan.infrastructure.util.KafkaDeserializer;
...
public class TimetablePeriodConsumer {
    ...
    @KafkaListener(
        ...
        groupId = "${trainjourneyplan.kafka.group-id}",
        topicPartitions = @org.springframework.kafka.annotation.TopicPartition(
            topic = "${trainjourneyplan.kafka.timetable-period-topic}",
            partitionOffsets = {@PartitionOffset(partition = "0", initialOffset = "0")}
        ),
        autoStartup = "${trainjourneyplan.kafka.enabled:true}"
    )
