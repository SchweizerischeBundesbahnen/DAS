...
const appVersion: AppVersion = { id: 1, version: '0.2.1', minimalVersion: false };
...
describe('AppVersionsService', () => {
  ...
  it('add should create app version', async () => {
    ...
    const successToastSpy = vi.spyOn(mockToastService, 'success');
    mockDialogResult({ version: '2.4.0', minimalVersion: true });

    await service.add();

    expect(successToastSpy).toHaveBeenCalled();
    expect(apiSpy).toHaveBeenCalledWith({ version: '2.4.0', minimalVersion: true });
  });
});
