...
test.describe('ru admin special holidays test', () => {
  ...
  async function deleteSpecialHoliday(page: Page, row: Locator) {
    await openEditDialog(row);
    const deleteResponse = page.waitForResponse((resp) =>
      resp.request().method() === 'DELETE' && resp.url().includes('/v1/special-holidays/'),
    );
  ...
  async function createSpecialHoliday(page: Page, holidayName: string) {
    ...
    const dateInput = page.locator('#date').last();
  ...
  test('create, edit and delete special holiday | tests: 1656', async ({page}) => {
    ...
    if (await row.isVisible()) {
      await deleteSpecialHoliday(page, row);
    }
    if (await updatedRow.isVisible()) {
      await deleteSpecialHoliday(page, updatedRow);
    }
  ...
  test('delete selected special holidays | tests: 1656', async ({page}) => {
    const row = findRow(page, TEST_HOLIDAY_NAME);
    const updatedRow = findRow(page, TEST_HOLIDAY_NAME_UPDATED);
